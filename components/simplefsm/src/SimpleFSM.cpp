...
const char* SimpleFSM::getErrorString(FSMError error) const
{
    ...
    case FSMError::OK:
        return "No error";
    case FSMError::OUT_OF_MEMORY:
        return "Out of memory";
    case FSMError::INVALID_PARAMETER:
        return "Invalid parameter";
    case FSMError::ARRAY_TOO_LARGE:
        return "Array size exceeds limits";
    default:
...
bool SimpleFSM::trigger(int event_id)
{
    ...
    for (int i = 0; i < num_standard; i++) {
        ...
        if ((transitions[i].from == NULL || transitions[i].from == current_state) &&
...
FSMError SimpleFSM::addGlobalTransition(State* to, int event_id, CallbackFunction callback)
{
    ...
    // isDuplicate() is not used here because it also matches on 'to', which we intentionally
    // ignore.
    if (transitions != NULL) {
        for (int i = 0; i < num_standard; i++) {
...
bool SimpleFSM::isDuplicate(const TimedTransition& transition,
                            const TimedTransition* transitionArray, int arraySize) const
{
    for (int i = 0; i < arraySize; ++i) {
        if (transitionArray[i].from == transition.from && transitionArray[i].to == transition.to &&
...
bool SimpleFSM::isDuplicate(const Transition& transition, const Transition* transitionArray,
                            int arraySize) const
{
    for (int i = 0; i < arraySize; ++i) {
        if (transitionArray[i].from == transition.from && transitionArray[i].to == transition.to &&
...
{
    return (last_transition == TIMESTAMP_RESET_VALUE) ? TIMESTAMP_RESET_VALUE
                                                      : (millis() - last_transition);
}
...
void SimpleFSM::run(int interval /* = DEFAULT_RUN_INTERVAL_MS */,
...
void SimpleFSM::handleTimedEvents(unsigned long now)
{
    for (int i = 0; i < num_timed; i++) {
        ...
        if (timed[i].from != NULL && timed[i].from != current_state)
            continue;
...
{
    for (int i = 0; i < num_standard; i++) {
        if (transitions[i].fromStateName != "")
            transitions[i].from = getStateByName(transitions[i].fromStateName);
        if (transitions[i].toStateName != "")
            transitions[i].to = getStateByName(transitions[i].toStateName);
    }

    for (int i = 0; i < num_timed; i++) {
        if (timed[i].fromStateName != "")
            timed[i].from = getStateByName(timed[i].fromStateName);
        if (timed[i].toStateName != "")
            timed[i].to = getStateByName(timed[i].toStateName);
    }
}
...
bool SimpleFSM::changeToState(State* s, unsigned long now)
{
    ...
    // is this the end?
    if (s->is_final && finished_cb != NULL)
        finished_cb();
    if (s->is_final)
        is_finished = true;
    return true;
...
{
    return "digraph G {\n" + getDOTHeader() + dot_definition +
           (showActive ? getDOTActiveNode() : "") + getDOTInitialState() + "}\n";
}
...
bool SimpleFSM::transitionTo(AbstractTransition* transition)
{
    ...
    } else if (transition->from == NULL && current_state != NULL &&
               current_state->on_exit != NULL) {
...
{
    return initial_state ? "\t\"" + initial_state->getName() +
...
String SimpleFSM::getDOTActiveNode()
{
    return current_state
               ? "\t\"" + current_state->getName() + "\" [style=filled fontcolor=white];\n"
               : "";
}
...
void SimpleFSM::addDOTTransition(Transition& t)
{
    ...
    dot_definition = dot_definition + getDOTTransition(fromName, t.to->getName(), t.getName(),
